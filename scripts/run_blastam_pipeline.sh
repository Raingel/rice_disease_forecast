...
echo "[INFO] BLASTAM pipeline started at $(date -u '+%Y-%m-%dT%H:%M:%SZ')"
...
run_py "$ROOT_DIR/models/BLASTAM" "run_blastam.py"
run_py "$ROOT_DIR/models" "recent_forecast_organizer.py"
run_py "$ROOT_DIR/models" "crop_season_avg.py"

echo "[INFO] BLASTAM pipeline completed at $(date -u '+%Y-%m-%dT%H:%M:%SZ')"
